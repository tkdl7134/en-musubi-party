...
public interface PartyMapper {
    ...
    @Select("SELECT e.e_no, m.m_id, e.ep_type, m.m_fam_kanji, m.m_name_kanji, m.m_gender, m.m_birth FROM member m JOIN en_party e ON m.m_id = e.M_ID")
    List<PartyVO> getPartyMembers();

    @Select("select m_id, ep_selectedType from en_party")
    List<PartyVO> getSelectedType();

    @Select("select * from en_party")
    List<PartyVO> getAllParty();

    @Select("select ep_finalChoice from en_party")
